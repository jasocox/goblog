...
func (v View) BlogList(w http.ResponseWriter) error {
	...
	execers := []templateExecer{
		templateExecer{blog_list, v.blogs.First()},
		templateExecer{blog_links, v.blogs.Last()},
	}
...
func (v View) doExecTemplate(name string, w http.ResponseWriter, exec func() error) error {
	v.log.Info("%s page", name)

	err := execLayout(w, exec)
